...
#
#
#Here we added the instruction iaddl to our pipeline in order to avoid the need of loading an immediate value as one instruction and adding the values in the next instruction.
## IADDL DESCRIPTION
##   FETCH:
##       M1[PC] --> Icode:Ifun
##       M1[PC+1] --> rA:rB
